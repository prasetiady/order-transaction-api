...
class ErrorHandler @Inject() (
  ...
  override def onProdServerError(request: RequestHeader, exception: UsefulException) = {
    implicit val writer = PlayBodyParsing.anyToWritable[ErrorModel](contentType(request))
    Future.successful(InternalServerError(ErrorModel(500, exception.description)))
  }
